...
import { PencilRuler, Clock, BarChart3, Rocket } from 'lucide-react'
...
const STAT_CARD_CONFIG = [
  { key: 'reviewCount' as const, icon: 'edit', label: '批改数量', grad: 'linear-gradient(135deg, #6366f1 0%, #8b5cf6 100%)', shadow: 'rgba(99,102,241,.25)' },
  { key: 'totalDuration' as const, icon: 'clock', label: '总评审时长', grad: 'linear-gradient(135deg, #10b981 0%, #059669 100%)', shadow: 'rgba(16,185,129,.25)' },
  { key: 'avgScore' as const, icon: 'chart', label: '平均评分', grad: 'linear-gradient(135deg, #f59e0b 0%, #d97706 100%)', shadow: 'rgba(245,158,11,.25)' },
  { key: 'publishRate' as const, icon: 'rocket', label: '推荐发行率', grad: 'linear-gradient(135deg, #ec4899 0%, #db2777 100%)', shadow: 'rgba(236,72,153,.25)' },
]
...
function MonthlyLineChart({ data }: { data: { month: string; count: number }[] }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H + 24}`} width="100%" style={{ display: 'block' }}>
      ...
      {[0, 0.25, 0.5, 0.75, 1].map((ratio) => {
        ...
        return (
          <g key={ratio}>
            ...
            <text x={padX - 8} y={y + 4} textAnchor="end" fontSize="10" fill="var(--text3)">{label}</text>
          </g>
        )
      })}
      {/* Area */}
      ...
      {points.map((p, i) => (
        <g key={i}>
          <circle cx={p.x} cy={p.y} r="4" fill="var(--bg3)" stroke="#6366f1" strokeWidth="2" />
          {data[i].count > 0 && (
            <text x={p.x} y={p.y - 10} textAnchor="middle" fontSize="10" fontWeight="600" fill="#6366f1">
              {data[i].count}
            </text>
          )}
          <text x={p.x} y={H + 14} textAnchor="middle" fontSize="10" fill="var(--text3)">
            {data[i].month}
          </text>
        </g>
      ))}
    ...
    <div className={pageWrap}>
      ...
      <div className="grid grid-cols-4 gap-3.5">
        {STAT_CARD_CONFIG.map((s, i) => {
          const IconComp = ICON_MAP[s.icon] || PencilRuler
          return (
            <div
              key={i}
          ...
          <thead>
            <tr className="border-b border-[var(--border)]">
              {COLUMNS.map((col) => (
                <th
                  key={col}
                  className="px-[18px] py-[13px] text-left text-xs font-semibold text-[var(--text3)] bg-[var(--bg4)] whitespace-nowrap"
                >
                  {col}
                </th>
              ))}
          ...
          <div className="px-5 py-3 flex justify-between items-center border-t border-[var(--border)] bg-[var(--bg4)]">
            ...
            <div className="flex items-center gap-[3px]">
              <button
                ...
                disabled={page === 1}
                className="w-7 h-7 border border-[var(--border)] rounded-md bg-[var(--bg3)] flex items-center justify-center text-[13px]"
                style={{
