...
export const POST = safeHandler(async function POST(request: NextRequest) {
  ...
  const { songId, lyrics, melody, arrangement, styleCreativity, commercial, tags, comment, recommendation, durationSeconds } = body

  if (!songId || lyrics == null || melody == null || arrangement == null || styleCreativity == null || commercial == null || !comment || !recommendation) {
    return err('缺少必填字段')
  }

  // 校验分数范围（必须为 0-100 的整数）
  if ([lyrics, melody, arrangement, styleCreativity, commercial].some((v) => typeof v !== 'number' || !Number.isInteger(v) || v < 0 || v > 100)) {
  ...
  const totalScore = Math.round(lyrics * 0.15 + melody * 0.15 + arrangement * 0.20 + styleCreativity * 0.20 + commercial * 0.30)
