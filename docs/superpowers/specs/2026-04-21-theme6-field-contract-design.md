...
| GAP-ADMIN-029 | admin/songs publish action | `Math.random()` 选号不保证年度唯一递增 | 新增 `copyright_sequences` 表 + `nextCopyrightNo(tx)` |
...
4. 版权编号年度连续递增：`T-D2` 并发 5 次 publish 得到 5 个互不相同且步长 1 的序号
...
- `src/app/api/creator/songs/route.ts` POST 路由：解析 body 后先 `fillSongDefaults(body, user)` 再 create
...
**新增 `src/lib/copyright-seq.ts`**（~25 行）：

```typescript
import type { Prisma } from '@prisma/client'

export async function nextCopyrightNo(tx: Prisma.TransactionClient): Promise<string> {
  ...
  return `${year}${String(next).padStart(6, '0')}`  // 2026000001
}
```

**调用点**：`src/app/api/admin/songs/[id]/status/route.ts` publish 分支。Theme 4 已把 publish 包进 `prisma.$transaction(async (tx) => {...})`，此处复用 tx 传入 `nextCopyrightNo(tx)`，把原 `Math.random()` 生成行替换即可。
...
管理员点"确认发行"
  ...
  ↓ prisma.$transaction(async tx => {
      ├─ 校验 签约/实名/ISRC（已存在）
      ├─ copyrightNo = await nextCopyrightNo(tx)
...
| `src/app/api/creator/songs/route.ts` | C | +1 helper 调用 |
| `src/app/api/creator/assignments/[id]/submit/route.ts` | C | +1 helper 调用 |
...
| `src/app/api/admin/songs/[id]/status/route.ts` | D | 1 行替换 `Math.random` |
| `prisma/schema.prisma` | D | +8 行 model |

### 新建（3 个）

- `src/lib/song-defaults.ts` — fillSongDefaults helper (~25 行)
- `src/lib/copyright-seq.ts` — nextCopyrightNo(tx) helper (~25 行)
...
| T-D1 | publish 一首歌 → song.copyrightNo 形如 `^2026\d{6}$` |
| T-D2 | 并发 publish 5 首（Promise.all）→ 5 个序号互不相同且彼此步长 1 |
...
- `nextCopyrightNo`：在 `$transaction` 内，任何 SQL 失败回滚整个 publish（与现状等价，无 regress）
